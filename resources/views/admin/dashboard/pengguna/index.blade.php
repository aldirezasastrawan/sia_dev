		...
		<h1>
            ...
            <small>Control panel</small>
		      ...
		      <ol class="breadcrumb">
		        ...
		        <li class="active">Data Pengguna Sistem {{Auth::user()->role->namaRule}}</li>
		      </ol>
				    <meta charset="utf-8">
    ...
    <div class="row">
            <div class="col-xs-12">
              <div class="box">
                ...
                <div class=row>
                <div class="col-md-6">
                    <a href="{{ url('/register/aktif') }}" class="btn btn-success"><i class="fa fa-plus"></i> Siswa</a>
                    <a href="{{ URL::to('/register/guru') }}" class="btn btn-danger"><i class="fa fa-plus"></i> Guru</a>
                    <a href="{{ URL::to('/register') }}" class="btn btn-default" type="submit"><i class="fa fa-refresh"></i> Reset</a>
                </div>
                <div class="col-md-2">
                </div>
                <!-- Form Pencarian -->
                <div class="col-md-4">
                    {!! Form::open(['method'=>'GET','url'=>'caripengguna'])  !!}
                  ...
                  <table id="dataGuru" class="table table-bordered table-hover">
                    ...
                    <tbody>
                     @foreach($users as $data)
                      <tr>			
            						<td>{{ $data->id}}</td>
            						<td>@if($data->role->namaRule === 'Administrator')                               
                                          <span class="label label-primary"><i class="fa fa-check"> Admin </i></span>
                                        @elseif($data->role->namaRule === 'Pimpinan')
                                           <span class="label label-warning"><i class="fa fa-check-circle"> Pimpinan </i></span>
                                        @elseif($data->role->namaRule === 'Guru')
                                          <span class="label label-danger"><i class="fa fa-check-circle"> Guru </i></span>
                                       @elseif($data->role->namaRule === 'Wali')
                                      <span class="label label-info"><i class="fa fa-check-circle"> Wali Siswa </i></span>
                
                                        @endif
                        </td>
            						<td><span class="label label-success"><i class="fa fa-unlock-alt"> {{$data->username}} </i></span></td>
            						<td>
							             <a class="btn btn-warning btn-flat btn-sm" href="{{{ URL::to('register/'.$data->id.'/ubah') }}}">
	                         	<span class="label label-warning"><i class="fa fa-edit"> Edit </i></span></a>
	                        <a class="btn btn-danger btn-flat btn-sm" href="{{{  URL::to('register/hapus',[$data->id]) }}}" title="Hapus Akun" onclick="return confirm('Apakah anda yakin akan menghapus Data Pengguna {{{($data->id).' - '.$data->role->namaRule}}}?')"><i class="fa fa-trash"> Unreg </i></span>
                            </a>
	                    </td>
                      </tr>
                      @endforeach 
